...
export const Route = createFileRoute("/dereck")({
  ssr: false,
  head: () => ({
    meta: [
      { title: "ÆTERNUM Peptides — Painel Dereck" },
      { name: "description", content: "Parceria e gestão de afiliados." },
    ],
  }),
  component: DereckPanel,
});

function DereckPanel() {
  const navigate = useNavigate();
  useEffect(() => {
    const r = getRole();
    if (r !== "dereck" && r !== "admin") navigate({ to: "/" });
  }, [navigate]);
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-lg font-serif font-bold gold-text">Painel Dereck</h1>
            </div>
          </div>
